...
type MrsClusterParameters struct {
	...
	// Specifies the network ID of a subnet which bound to the MRS cluster.
	// Changing this will create a new MRS cluster resource.
...
// MrsClusterSpec defines the desired state of MrsCluster
type MrsClusterSpec struct {
	v1.ResourceSpec `json:",inline"`
	ForProvider     MrsClusterParameters `json:"forProvider"`
}

// MrsClusterStatus defines the observed state of MrsCluster.
type MrsClusterStatus struct {
	v1.ResourceStatus `json:",inline"`
	AtProvider        MrsClusterObservation `json:"atProvider,omitempty"`
}

// +kubebuilder:object:root=true

// MrsCluster is the Schema for the MrsClusters API. ""page_title: "flexibleengine_mrs_cluster_v2"
...
type MrsCluster struct {
	...
	Spec              MrsClusterSpec   `json:"spec"`
	Status            MrsClusterStatus `json:"status,omitempty"`
}

// +kubebuilder:object:root=true

// MrsClusterList contains a list of MrsClusters
type MrsClusterList struct {
	metav1.TypeMeta `json:",inline"`
	metav1.ListMeta `json:"metadata,omitempty"`
	Items           []MrsCluster `json:"items"`
}

// Repository type metadata.
var (
	MrsCluster_Kind             = "MrsCluster"
	MrsCluster_GroupKind        = schema.GroupKind{Group: CRDGroup, Kind: MrsCluster_Kind}.String()
	MrsCluster_KindAPIVersion   = MrsCluster_Kind + "." + CRDGroupVersion.String()
	MrsCluster_GroupVersionKind = CRDGroupVersion.WithKind(MrsCluster_Kind)
)

func init() {
	SchemeBuilder.Register(&MrsCluster{}, &MrsClusterList{})
}
